...
def _generate_key_messages(
    ...
    if slide_type == SlideType.BACKGROUND:
        ...
        if sections:
            return _extract_lead_sentences(sections[0].paragraphs, count=3)
        return ["Context and background of the research problem domain."]

    # 3. SlideType: PROBLEM
    if slide_type == SlideType.PROBLEM:
        if sections:
            sents = _extract_lead_sentences(sections[0].paragraphs, count=3)
            if sents:
                return sents
        return [
...
def _match_sections_for_category(
    ...
    matched = []
    # Primary match: exact category match
    for r in ranked:
        sec = r.section
    ...
    # Secondary match: reuse already used section if exact category exists
    for r in ranked:
        if r.category == category:
            matched.append(r.section)
            return matched
    ...
    # Ultimate fallback: return first high-scoring section
    if ranked:
        matched.append(ranked[0].section)
    return matched
